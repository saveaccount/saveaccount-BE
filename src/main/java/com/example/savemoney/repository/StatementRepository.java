...
public interface StatementRepository extends JpaRepository<Statement, Long> {
    ...
    // Custom query to calculate the total spending of a user in the last month
    @Query("SELECT COALESCE(SUM(s.amount), 0) FROM Statement s WHERE s.user.id = :userId AND s.createdAt >= :oneMonthAgo")
